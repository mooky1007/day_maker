...
const TodoList = ({ type, data, setData }) => {
    ...
    const getTodoList = () => {
        axios.get('http://43.201.253.26:3001/todo').then((res) => {
            const { data } = res;
            setData(data);
        });
    };
    ...
    const deleteTodo = async (key) => {
        const { status } = await axios.delete(`http://43.201.253.26:3001/delete_todo?key=${key}`);
        if (status === 200) setData((prev) => prev.filter((el) => el.key !== key));
    };
    const updateTodo = async (key, data) => {
        const { status } = await axios.put(`http://43.201.253.26:3001/update_todo`, data);
        if (status === 200)
            setData((prev) =>
                prev.map((el) => {
